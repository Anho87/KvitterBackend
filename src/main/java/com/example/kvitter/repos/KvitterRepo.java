...
public interface KvitterRepo extends JpaRepository<Kvitter, UUID> {
    ...
    @Query(value = "SELECT DISTINCT k.* FROM Kvitter k LEFT JOIN kvitter_hashtags kv on k.id = kv.kvitter_id left join Hashtag h on h.id = kv.hashtag_id where (h.hashtag ILIKE  :searched and k.is_private = false) OR (h.hashtag ILIKE :searched and k.is_private = true and k.user_id = :activeId)", nativeQuery = true)
